...
from app.core.settings import create_access_token, create_refresh_token, decode_refresh_token, verify_password
from app.repositories.refresh_token import RefreshTokenRepository
from app.repositories.user import UserRepository
from app.schemas.auth import LoginSchema, TokenRefreshSchema, TokenRevokeSchema, TokenSchema
...
class AuthService:
    ...
    def login(self, payload: LoginSchema) -> TokenSchema:
        ...
        if not verify_password(plain=payload.password.get_secret_value(), hashed=user.password):
            raise ValueError("Неверный login или password")
        ...
        self.refresh_repository.create(user_id=user.id, token=refresh_token, expires_at=expires_at, is_revoked=False)
        self.db.commit()
    ...
    def refresh_token(self, token_data: TokenRefreshSchema) -> TokenSchema:
        ...
        if refresh_orm.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
            raise ValueError("Refresh-токен истёк")
        
        user_id = payload.get("sub")
        access_token_data = {"sub": user_id}
        new_access = create_access_token(access_token_data)
        return TokenSchema(access_token=new_access, refresh_token=token_data.refresh_token)
